...
public class init_AntDFT_gen_band {
     ...
     */
    public parameter main(int ik, double campur, parameter p_d, init_data init, int num) throws InterruptedException, IOException, ClassNotFoundException, JSchException, SftpException {
        Date date = new Date();
        ...
        if (num == 0) {
            ...
            outputStream = new ObjectOutputStream(new FileOutputStream(new config_folder().url_out + "datam"));
            outputStream.writeObject(param);
            outputStream.close();
        } else {
            ObjectInputStream inputStream;
            inputStream = new ObjectInputStream(new FileInputStream(new config_folder().url_out + "datam"));

            param = (parameter) inputStream.readObject();
            inputStream.close();
        }
