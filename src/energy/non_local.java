...
public class non_local {
    ...
    public void main(parameter param, int ik) {
        ...
        for (int j = 0; j < param.nat; j++) {
            for (int i = 0; i < j; i++) {
                int in = param.upf_data.get(param.atom_p[i]).deeq[j].length;
                index[j] += in;
            }
        }
        for (int j = 0; j < param.nat; j++) {
            int in = param.upf_data.get(param.atom_p[j]).deeq[j].length;
        ...
        for (int nt = 0; nt < param.banyak_atom; nt++) {
            for (int i = 0; i < bec.length; i++) {
                if (param.atom_p[i].equals(param.atom[nt])) {
                    bec[i] = ao.time_complex(param.upf_data.get(param.atom_p[i]).deeq[i], bec[i]);
                }
            }
        }
        ...
        param.non_local.put(ik, non_loc);
